...
It actually loads and displays the first 10 observations in the dataset.
...
college_recent_grads %>%
  summarise(mean=mean(median), stddev=sd(median), minimum=min(median), median=median(median),maximum=max(median))
...
The data appears to be somewhat right skewed with a sizable standard deviation.
...
It appeasr that in general STEM categories and business have the largest salaries overall while social sciences and social work appear to be lower on the list. Although it is important to note that there is decent spread on most of the majors salaries.
...
It put the data althogether on a single paged table. This would be helpful when getting a final table for the data as it no longer needs to be changed.
...
My descriptions appeared to fit pretty well with what the graphs show
...
```{r major_category, message=FALSE}

college_recent_grads %>%
  group_by(major_category) %>%
  summarise(total=length(major), percentwomen=median(sharewomen),unemployed=median(unemployment_rate),median=median(median)) %>%
  mutate(numrank=min_rank(total),sharewomenrank=min_rank(percentwomen),unemployedrank=min_rank(unemployed)) %>%
...
Women tend to be in jobs that have lower wages and higher unemployment overall than men do.
